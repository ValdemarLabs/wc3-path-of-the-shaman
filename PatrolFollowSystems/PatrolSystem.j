...
globals
    ...
    // Debug flag
    private boolean PS_DEBUG = false  // Set to true to enable debug messages
endglobals
...
//   50 : suppress (boolean)
//   51 : paused state before manual pause
//   1000+idx : x (waypoint)
//   2000+idx : y (waypoint)
...
private function PreviewNextIndex takes integer id, integer idx returns integer
    local integer count = LoadInteger(ht, id, 6)
    local integer style = LoadInteger(ht, id, 8)
    local integer dir = LoadInteger(ht, id, 5)

    if style == PATROL_STYLE_LOOP then
        return ModuloInteger(idx + 1, count)
    endif
...
private function GetNearestWaypointIndex takes unit u returns integer
    local integer id = GetHandleId(u)
    local integer count = LoadInteger(ht, id, 6)
    local integer idx = 0
    local integer bestIdx = 0
    ...
    loop
        exitwhen idx >= count
        set dx = GetUnitX(u) - LoadReal(ht, id, 1000 + idx)
        set dy = GetUnitY(u) - LoadReal(ht, id, 2000 + idx)
...
private function GroupTimerExpire takes nothing returns nothing
    ...
    if state == 1 then
        ...
        // Find first living unit to check distance (skip dead units)
        set u = null
        set i = 0
        loop
            exitwhen i >= unitCount
        ...
        if u != null then
            ...
            if PS_DEBUG then
                call BJDebugMsg("[GroupTimerExpire] First living unit distance: " + R2S(dist) + ", threshold=" + R2S(GROUP_EPSILON))
            endif
        else
            if PS_DEBUG then
                call BJDebugMsg("[GroupTimerExpire] ERROR: No living units in group!")
            endif
            return
        endif
...
private function OnDamage takes nothing returns nothing
    ...
    if victim != null then
        ...
        if groupId > 0 then
            // Only interrupt if NOT already in reset or paused state
            if LoadInteger(grpHt, groupId, 4) == 1 or LoadInteger(grpHt, groupId, 4) == 2 then
                // Pause entire group
                ...
                // Get victim's position for group to converge on
                set targetX = GetUnitX(victim)
                set targetY = GetUnitY(victim)
                
                // Order all units to assist at victim's position
                ...
                loop
                    ...
                    if u != null and GetUnitTypeId(u) != 0 then
                        call SetUnitMoveSpeed(u, GetUnitDefaultMoveSpeed(u))
                        call SaveBoolean(ht, GetHandleId(u), 50, true)
                        // Order to attack-move to victim's position
                        call IssuePointOrderById(u, 851983, targetX, targetY)  // attack-move order
                    endif
                    set i = i + 1
                endloop
        ...
        elseif HaveSavedHandle(ht, id, 0) then
            // Single unit patrol - only interrupt if actively patrolling
            if LoadInteger(ht, id, 4) == 1 or LoadInteger(ht, id, 4) == 2 then
                set t = LoadTimerHandle(ht, id, 1)
                if t != null then
                    call PauseTimer(t)
                endif
    ...
    if attacker != null then
        ...
        if groupId > 0 then
            ...
            if LoadInteger(grpHt, groupId, 4) == 1 or LoadInteger(grpHt, groupId, 4) == 2 then
                ...
                // Get attacker's position for group to converge on
                set targetX = GetUnitX(attacker)
                set targetY = GetUnitY(attacker)
                
                // Order all units to assist at attacker's position
                set unitCount = LoadInteger(grpHt, groupId, 11)
                set i = 0
                loop
                    exitwhen i >= unitCount
                    ...
                    if u != null and GetUnitTypeId(u) != 0 then
                        call SetUnitMoveSpeed(u, GetUnitDefaultMoveSpeed(u))
                        call SaveBoolean(ht, GetHandleId(u), 50, true)
                        // Order to attack-move to attacker's position
                        call IssuePointOrderById(u, 851983, targetX, targetY)  // attack-move order
                    endif
                    set i = i + 1
                endloop
...
function PatrolSystem_Pause takes unit u returns nothing
    ...
    // mark paused
    call SaveInteger(ht, id, 51, state)
    call SaveInteger(ht, id, 4, 0) // state = paused
endfunction

// Pauses patrol state/timer without issuing a stop order.
// Use this when another system is about to relocate the unit immediately.
function PatrolSystem_PauseForRelocation takes unit u returns nothing
    local integer id = GetHandleId(u)
    local timer t = LoadTimerHandle(ht, id, 1)
    local integer state = 0
...
function PatrolSystem_IsActive takes unit u returns boolean
    local integer id
...
function PatrolSystem_ResumeFromCurrentPositionEx takes unit u, boolean skipCurrentWaypoint returns nothing
    local integer id = GetHandleId(u)
    local integer state
    local integer pausedState
    local integer count
    local integer idx
    ...
    local real nextDx
    local real nextDy
    local real currentDistanceSq
    local real nextDistanceSq
    local string moveOrder
    ...
    set pausedState = LoadInteger(ht, id, 51)
    set count = LoadInteger(ht, id, 6)
    set idx = LoadInteger(ht, id, 2)
    if pausedState == 1 and skipCurrentWaypoint then
        set idx = NextIndex(id, idx)
    elseif pausedState == 1 then
        set steps = 0
        loop
            exitwhen steps >= count
            set nextIdx = PreviewNextIndex(id, idx)
            exitwhen nextIdx == idx

            set dx = GetUnitX(u) - LoadReal(ht, id, 1000 + idx)
            set dy = GetUnitY(u) - LoadReal(ht, id, 2000 + idx)
            set nextDx = GetUnitX(u) - LoadReal(ht, id, 1000 + nextIdx)
            set nextDy = GetUnitY(u) - LoadReal(ht, id, 2000 + nextIdx)
            set currentDistanceSq = dx*dx + dy*dy
            set nextDistanceSq = nextDx*nextDx + nextDy*nextDy

            // When relocation leaves the unit clearly closer to later legs,
            // keep advancing until the stale target is no longer behind it.
            if nextDistanceSq + EPSILON*EPSILON < currentDistanceSq then
                set idx = NextIndex(id, idx)
                set steps = steps + 1
            else
                exitwhen true
        ...
        endloop
    elseif pausedState == 2 or pausedState == 3 then
        set idx = NextIndex(id, idx)
    endif
    ...
    loop
        ...
        set nextIdx = NextIndex(id, idx)
        exitwhen nextIdx == idx
        set idx = nextIdx
        set steps = steps + 1
    endloop

    set moveOrder = LoadStr(ht, id, 9)
    call SaveInteger(ht, id, 2, idx)
    call SaveInteger(ht, id, 51, 0)
    call SaveBoolean(ht, id, 50, true)
    call IssuePointOrder(u, moveOrder, x, y)
    call StartTimerFor(u, MoveTime(u, x, y), 1)
endfunction

function PatrolSystem_ResumeFromCurrentPosition takes unit u returns nothing
    call PatrolSystem_ResumeFromCurrentPositionEx(u, false)
endfunction
