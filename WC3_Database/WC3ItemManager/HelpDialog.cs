...
namespace WC3ItemManager
{
    public class HelpDialog : Form
    {
        ...
        private TabPage CreateFAQTab()
        {
            ...
            AppendFAQ(rtb, "How do I limit an item to explicit drops?",
                "Enable Specific Drops Only on the item, or batch edit the same flag for selected items. Generic loot pools and loot-table autofill skip flagged items, while enabled unit-specific drops still export.");
            ...
            AppendFAQ(rtb, "How do I require a quest for a unit-specific drop?",
                "Open the unit type or the item's Drop Sources tab, then select a Quest gate and Required state. Active rolls only while the shared QuestMaster quest is active and unfinished; Discovered rolls after the quest has been revealed. The Drop Sources grid supports direct editing and Ctrl/Shift multi-selection through Bulk Edit. Export the Loot System JASS after saving.");
