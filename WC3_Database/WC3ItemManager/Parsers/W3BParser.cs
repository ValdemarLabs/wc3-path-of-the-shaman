...
namespace WC3ItemManager.Parsers
{
    ...
    public class W3BDestructible
    {
        ...
        public float? RepairTimeRaw { get; set; }       // WE field bret (Stats - Repair Time)
        public int? LootLevelFromWE { get; set; }       // Rounded value of bret used as loot level
    ...
    /// - bsuf = editor suffix
    /// - bret = repair time (repurposed as loot level for destructibles)
    /// </summary>
            ...
            if (!string.IsNullOrEmpty(fieldId))
            {
                ...
                switch (fieldId.ToLowerInvariant())
                {
                    ...
                    case "bsuf": // Editor suffix (Text - Editor Suffix)
                        ...
                        break;
                    case "bret": // Stats - Repair Time (used as loot level proxy)
                        if (value is float realValue)
                        {
                            dest.RepairTimeRaw = realValue;
                            dest.LootLevelFromWE = Math.Max(0, (int)Math.Round(realValue, MidpointRounding.AwayFromZero));
                        }
                        else if (value is int intValue)
                        {
                            dest.RepairTimeRaw = intValue;
                            dest.LootLevelFromWE = Math.Max(0, intValue);
                        }
                        break;
                }
            }
