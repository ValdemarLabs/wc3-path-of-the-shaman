"""Compatibility wrapper for the legacy W3A importer path used by ItemManager."""
...
def _load_archive_module() -> ModuleType:
    ...
    archive_script = database_root / "_Archive" / "importers" / "wc3_w3a_importer.py"

    if str(database_root) not in sys.path:
        sys.path.insert(0, str(database_root))

    spec = importlib.util.spec_from_file_location(
        "pots_archive_wc3_w3a_importer",
        archive_script,
    )
...
WC3AbilityImporter = _archive.WC3AbilityImporter
WC3W3AImporter = WC3AbilityImporter
