...
namespace WC3ItemManager
{
    ...
    public class TooltipGenerator
    {
        ...
        {
            var closingOptions = new Dictionary<string, string[]>
            {
                { "Common", new[] { "A reliable piece of equipment.", "Sturdy and practical.", "Simple, yet effective." } },
                { "Uncommon", new[] { "Well-crafted and dependable.", "Finely made and useful.", "A notch above the ordinary." } },
                { "Rare", new[] { "A treasure worth protecting.", "A find most fortunate.", "Rare and valuable indeed." } },
                { "Epic", new[] { "The stuff of legends.", "A relic of great power.", "Few items match its majesty." } },
                { "Legendary", new[] { "Few have wielded such power.", "A legend made manifest.", "The pinnacle of craftsmanship." } }
            };
            
            if (closingOptions.ContainsKey(rarity))
            {
                var options = closingOptions[rarity];
                // Use time-based seed for better randomness
                int index = (new Random(DateTime.Now.Millisecond + DateTime.Now.Second * 1000 + DateTime.Now.Minute * 60000)).Next(options.Length);
        ...
        {
            var loreMap = new Dictionary<string, string>
            {
                { "str", "Imbued with raw physical might, this item channels the strength of ancient warriors." },
                { "agi", "Swift as the wind, this artifact enhances the bearer's grace and precision." },
                { "int", "Crackling with arcane energy, this relic amplifies the wielder's magical prowess." },
                { "hp", "Blessed with vitality, this item fortifies the life force of its bearer." },
                { "mp", "Resonating with magical essence, this artifact expands one's reserves of power." },
                { "dmg", "Forged in battle, this weapon thirsts for combat and devastation." },
                { "armor", "Hardened through countless conflicts, this protection stands unyielding." },
                { "crit", "Sharp and deadly, this item enhances precision strikes with lethal efficiency." },
                { "lifesteal", "Dark energies swirl within, siphoning life from those it vanquishes." },
                { "spell_power", "Ancient runes glow with power, amplifying magical forces beyond mortal limits." }
            };

            return loreMap.ContainsKey(statCode) ? 
                loreMap[statCode] : 
                "An item of considerable power, its true nature remains mysterious.";
        }

        private string GetFlavorTextForRarity(string rarity)
        {
            // Use static Random to avoid seed issues when called in rapid succession
            var flavorOptions = new Dictionary<string, string[]>
            {
                { "Common", new[] { 
            ...
            if (flavorOptions.ContainsKey(rarity))
            {
                var options = flavorOptions[rarity];
                // Use a static Random with a time-based seed to avoid rapid succession issues
                int index = (new Random(DateTime.Now.Millisecond + DateTime.Now.Second * 1000)).Next(options.Length);
                return options[index];
            }
