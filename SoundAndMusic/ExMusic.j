...
/*
    ...
    How to use:
    1. Enable local files for Warcraft III
    2. Place your mp3 music files in the appropriate directory within your Warcraft III installation.
    3. Update the Init function to register your music tracks by adding their file paths to the ms_MusicPaths array.
        - Ensure the paths are correct and the files exist.
        - Currently requires manual work to add tracks
    4. Call ExMusic_PreloadAll() once at the start of your map to preload the music system.
    5. Use the provided API functions to control music playback during your game.

    API:
        call ExMusic_PreloadAll() - Preloads the music system and registers tracks.
        call ExMusic_PlayTrack(index) - Plays the track at the specified index with a fade transition.
        call ExMusic_PlayTrackImmediate(index) - Instantly plays the track at the specified index, cutting off any current music.
        ...
        call ExMusic_GetCurrentTrackIndex() - Returns the index of the currently playing track, or -1 if none.
        call ExMusic_GetCurrentTrack() - Updates a global string variable with the name of the currently playing track.
...
//=================================================================
globals
    private constant boolean DEBUG = false // set to false to disable debug messages
    private constant integer MUSIC_IDLE      = 0    // music states
    private constant integer MUSIC_FADING    = 1    
    ...
    private integer ms_PendingTrack = -1            // track to play after fade
    private integer ms_MusicState = MUSIC_IDLE      // current music state

    private timer ms_MusicTimer = null
...
//=================================================================
private function ExMusic_FadeComplete takes nothing returns nothing
    local string track
    ...
    // Nothing queued?
    if ms_PendingTrack < 0 or ms_PendingTrack >= ms_MusicCount then
        set ms_MusicState = MUSIC_IDLE
        if DEBUG then
            call BJDebugMsg("ExMusic_FadeComplete: Invalid or missing track. Fallback to game music.")
        endif
        call ResumeMusicBJ() // fallback to game music
        return
    endif

    set track = ms_MusicPaths[ms_PendingTrack]
    call PlayMusic(track)
    set ms_CurrentTrack = ms_PendingTrack
    set ms_PendingTrack = -1
    set ms_MusicState = MUSIC_IDLE
    if DEBUG then
        call BJDebugMsg("ExMusic_PlayTrack called: Playing track -> " + track)
    endif
endfunction
...
function ExMusic_PlayTrack takes integer index returns nothing
     // Skip music if cinematic trailer is active (PotS specific)
    if udg_CinematicTrailer then
        return
    endif
    ...
    if index < 0 or index >= ms_MusicCount then
        if DEBUG then
            ...
            call BJDebugMsg("ExMusic_PlayTrack: Fallback to game music.")
        endif
        call ResumeMusicBJ() // fallback to game music
        return
    endif
    ...
    // Queue the track
    set ms_PendingTrack = index

    // Already fading? Just replace pending track
    if ms_MusicState == MUSIC_FADING then
        return
    endif

    // Begin fade
    set ms_MusicState = MUSIC_FADING
    call StopMusicBJ(true)

    // Timer instead of sleep
    if ms_MusicTimer == null then
        set ms_MusicTimer = CreateTimer()
    endif

    // 2.0 matches Blizzard fade duration; can be tuned
    call TimerStart(ms_MusicTimer, 2.00, false, function ExMusic_FadeComplete)
endfunction
...
//=================================================================
// Play with instant cutoff of previous track
//=================================================================
function ExMusic_PlayTrackImmediate takes integer index returns nothing
    call StopMusicBJ(false)
    call ClearMapMusicBJ()
    if index < 0 or index >= ms_MusicCount or GetSoundFileDuration(ms_MusicPaths[index]) <= 0 then
        if DEBUG then
            call BJDebugMsg("ExMusic_PlayTrackImmediate: Invalid or missing track. Fallback to game music.")
        endif
        call ResumeMusicBJ() // fallback to game music
    else
        if DEBUG then
            call BJDebugMsg("ExMusic_PlayTrackImmediate: Playing track -> " + ms_MusicPaths[index])
        endif
        call PlayMusic(ms_MusicPaths[index])
    endif
endfunction
