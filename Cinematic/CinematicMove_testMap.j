...
//===========================================================================
    ...
    globals
        // Constants
        private constant real MAX_MOVE_RANGE        = 1200.0  // Skip moving companions/pets if already within this range
        private constant real MAX_RETURN_RANGE      = 1200.0  // Skip returning units if they were within this range initially
        private constant real RANDOM_OFFSET         = 250.0
        private constant real OFF_MAP_CORNER_X      = -15000.0  // Off-map corner X for clean death animations
        private constant real OFF_MAP_CORNER_Y      = -15000.0  // Off-map corner Y for clean death animations
...
//===========================================================================
    ...
    private function HandleUnitReturn takes unit u, real cx, real cy returns nothing
        ...
        if WasRevived(u) then
            ...
            if IsUnitInGroup(u, udg_TamedUnits) then
                ...
                //callcall TriggerExecute(gg_trg_Tamed_Unit_Dies)
            else
                call KillUnit(u)
            endif
    ...
    private function MoveCompanionCallback takes nothing returns nothing
        ...
        // Check if companion is already close to the cinematic trigger unit
        if data.real.has(id*2 + 100) then
            set dist = data.real[id*2 + 100]
    ...
    private function MoveTamedCallback takes nothing returns nothing
        ...
        // Check if pet is already close to the cinematic trigger unit
        if data.real.has(id*2 + 100) then
            set dist = data.real[id*2 + 100]
            if dist <= MAX_MOVE_RANGE then
    ...
    function CinematicMover_MoveUnitsToCinematic takes unit cineTriggerUnit, integer moveMode returns nothing
        ...
        */
        ...
        elseif moveMode == 9 then
            // Mode 9: Move all units (like mode 1) but don't return them
