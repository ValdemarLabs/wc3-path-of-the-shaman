...
library VendorDialogs initializer Init requires Table, DialogInteraction, DialogCamera, DialogSystem, Shop, ShopUI, VendorCatalogs, VendorOrcs, VendorSatyrs, VendorHumans, VendorGoblins, VendorBonecrusherOgres, VendorElarindor, VendorTauren, VendorDwarves, VendorTrolls, VendorLines, Reputation, Interface, optional QuestsVendor
    globals
        ...
        private constant boolean VDI_END_ON_COMBAT = true
    ...
    private function VDI_OnTrade takes nothing returns nothing
        ...
        if VDI_END_ON_COMBAT and (DialogInteraction_IsUnitInCombat(vendor) or DialogInteraction_IsUnitInCombat(hero)) then
            call Interface_PlayEventSoundForPlayer(Interface_EVENT_ERROR, Player(0))
        ...
        call ShowUnit(hero, true)
        set VDI_DialogCooldown = DialogInteraction_StartCooldown(VDI_DialogCooldown, VDI_DIALOG_COOLDOWN)
        call ShopUI_ShowForVendorWithReturnEx(vendor, hero, VDI_END_ON_COMBAT)
        set VDI_SelectedVendor = null
        set VDI_SelectedHero = null
    ...
    private function VDI_ReturnFromTrade takes nothing returns boolean
        ...
        if not DialogInteraction_BeginCombatSensitiveInteractionEx(vendor, hero, function VDI_InterruptDialog, VDI_END_ON_COMBAT) then
            set VDI_SelectedVendor = null
            set VDI_SelectedHero = null
            set vendor = null
            set hero = null
    ...
    private function VDI_OnSelected takes nothing returns nothing
        ...
        set gateOk = DialogInteraction_PassDialogSelectionGate(vendor, hero, VDI_DIALOG_RANGE, VDI_DialogCooldown, VDI_REQUIRE_DIALOG_HERO, true, false, true, false, true)
        if not gateOk then
