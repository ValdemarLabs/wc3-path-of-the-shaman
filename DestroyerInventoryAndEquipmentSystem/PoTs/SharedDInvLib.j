...
local integer currentCharges
local integer maxCharges
if bid < 1 then
// Could not find this unit in the DInventory system's list of heroes, so there is no inventory to check
else
    loop
        set existingItem = DInventoryDB[bid].item[loopi]
        if GetItemTypeId(existingItem) == iid then
        // Found item of same type
            if DInventoryIsItemStackable(existingItem) == TRUE then
            // Item is stackable - check if it has room for more charges
                set currentCharges = GetItemCharges(existingItem)
                set maxCharges = GetItemLevel(existingItem)
                if currentCharges < maxCharges then
                // Stack is not full, can add more items
////call BJDebugMsg("FirstStackableItemSlotOfBID returning "+I2S(loopi))
                    set it = null
                    set existingItem = null
                    return loopi
                endif
            endif
        endif
...
set it = null
set existingItem = null
// returns -1 if bid is not valid or no stackable slot with room found
return -1
endfunction
...
// Changed the order of texts (Valdemar)
if EQIDDB[eqid][5].real[1] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[1]) + " |c00FA8072Strength|r" + "|n"
endif

if EQIDDB[eqid][5].real[2] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[2]) + " Agility" + "|n"
endif

if EQIDDB[eqid][5].real[3] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[3]) + " |c0087CEEBIntelligence|r" + "|n"
endif

if EQIDDB[eqid][5].real[4] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[4]) + " |c00DC143CHitpoints|r" + "|n"
endif

if EQIDDB[eqid][5].real[5] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[5]) + " |c00DC143CHitpoint regeneration|r" + "|n"
endif

if EQIDDB[eqid][5].real[6] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[6]*100.0) + " %%" + " |c00DC143CHitpoint regeneration|r" + "|n"
endif

if EQIDDB[eqid][5].real[7] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[7]) + " |c001E90FFMana|r" + "|n"
endif

if EQIDDB[eqid][5].real[8] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[8]) + " |c001E90FFMana regeneration|r" + "|n"
endif

if EQIDDB[eqid][5].real[9] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[9]*100.0) + " %%" + " |c001E90FFMana regeneration|r" + "|n"
endif

if EQIDDB[eqid][5].real[10] == 0.0 then
else
//set s = s + "Crit: " + R2Dec2S(EQIDDB[eqid][5].real[10]*100.0)+ "%%|n"
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[10]) + " %%" + " |c00FFFF80Critical Chance|r" + "|n"
endif

if EQIDDB[eqid][5].real[11] == 0.0 then
else
set s = s + "+" + R2Dec2S((DefaultCritMultiplier + EQIDDB[eqid][5].real[11])*100.0) + " %%" + " |c00FFFF80Critical Damage|r" + "|n"
endif

if EQIDDB[eqid][5].real[12] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[12]) + " |c00FF6347Damage|r" + "|n"
endif

if EQIDDB[eqid][5].real[13] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[13]*100.0) + " %%" + " |c00FF6347Damage|r" + "|n"
endif

if EQIDDB[eqid][5].real[14] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[14]) + " |c00FF6347Melee Damage|r" + "|n"
endif

if EQIDDB[eqid][5].real[15] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[15]*100.0) + " %%" + " |c00FF6347Melee Damage|r" + "|n"
endif

if EQIDDB[eqid][5].real[16] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[16]) + " |c00FF6347Ranged Damage|r" + "|n"
endif

if EQIDDB[eqid][5].real[17] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[17]*100.0) + " %%" + " |c00FF6347Ranged Damage|r" + "|n"
endif

if EQIDDB[eqid][5].real[18] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[18]*100.0) + " %%" + " |c00FFFF80Cleave|r" + "|n"
endif

if EQIDDB[eqid][5].real[19] == 0.0 then
else
set s = s + "+" + R2Dec2S(150.0+EQIDDB[eqid][5].real[19]) + " |c00FFFF80Cleave Area|r" + "|n"
endif

if EQIDDB[eqid][5].real[20] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[20]*100.0) + " %%" + " Attack Speed" + "|n"
endif

if EQIDDB[eqid][5].real[21] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[21]) + " Attack Range" + "|n"
endif

if EQIDDB[eqid][5].real[22] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[22]*100.0) + " %%" + " Lifesteal" + "|n"
endif

if EQIDDB[eqid][5].real[23] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[23]) + " Thorns" + "|n"
endif

if EQIDDB[eqid][5].real[24] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[24]*100.0) + " %%" + " Thorns" + "|n"
endif

if EQIDDB[eqid][5].real[25] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[25]) + " Armor" + "|n"
endif

if EQIDDB[eqid][5].real[26] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[26]*100.0) + " %%" + " Armor" + "|n"
endif

if EQIDDB[eqid][5].real[27] == 0.0 then
else
//set s = s + "Evasion: " + R2Dec2S(EQIDDB[eqid][5].real[27]*100.0)+ "%%|n"
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[27]) + " %%" + " |c00FFFF80Dodge Chance|r" + "|n"
endif

if EQIDDB[eqid][5].real[28] == 0.0 then
else
    if 1+EQIDDB[eqid][5].real[28] < MagicDMGTakenPctLowCap then
    set s = s + "+" + R2Dec2S(MagicDMGTakenPctLowCap) + " %%" + " |c00FFAA82Spell Damage Taken|r" + "|n"
    else
    set s = s + "+" + R2Dec2S((1.0+EQIDDB[eqid][5].real[28])*100.0) + " %%" + " |c00FFAA82Spell Damage Taken|r" + "|n"
    endif
endif
...
if EQIDDB[eqid][5].real[29] == 0.0 then
    ...
    set s = s + "+" + R2Dec2S(MeleeDMGTakenPctLowCap) + " %%" + " Melee Damage Taken" + "|n"
    else
    set s = s + "+" + R2Dec2S((1.0+EQIDDB[eqid][5].real[29])*100.0) + " %%" + " Melee Damage Taken" + "|n"
    endif
endif
...
if EQIDDB[eqid][5].real[30] == 0.0 then
    ...
    set s = s + "+" + R2Dec2S(PierceDMGTakenPctLowCap) + " %%" + " Pierce Damage Taken" + "|n"
    else
    set s = s + "+" + R2Dec2S((1.0+EQIDDB[eqid][5].real[30])*100.0) + " %%" + " Pierce Damage Taken" + "|n"
    endif
endif

if EQIDDB[eqid][5].real[31] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[31]) + " Movement Speed" + "|n"
endif

if EQIDDB[eqid][5].real[32] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[32]*100.0) + " %%" + " Movement Speed" + "|n"
endif

if EQIDDB[eqid][5].real[33] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[33]) + " Sight Range" + "|n"
endif

if EQIDDB[eqid][5].real[35] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[35]) + " %%" + " |c00FFFF80Block Chance|r" + "|n"
endif

if EQIDDB[eqid][5].real[36] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[36]) + " %%" + " |c00FFFF80Hit Chance|r" + "|n"
endif

if EQIDDB[eqid][5].real[37] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[37]) + " %%" + " |c00FFAA82Spell Power|r" + "|n"
endif

if EQIDDB[eqid][5].real[38] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[38]) + " |c00FFAA82Spell Power|r" + "|n"
endif

if EQIDDB[eqid][5].real[39] == 0.0 then
else
set s = s + "+" + R2Dec2S(EQIDDB[eqid][5].real[39]) + " %%" + " |c00FFAA82Healing Power|r" + "|n"
endif
...
//call BJDebugMsg("stat found")
    set amount = DEqItemTypeDefinitionDB[iid][10].real[i]
        ...
        // Apply coloring based on stat type
        if i == 1 then
            set statColor = "|c00FA8072" // Strength
        elseif i == 2 then
            set statColor = "|c00ADFF2F" // Agility
        elseif i == 3 then
            set statColor = "|c0087CEEB" // Intelligence
        elseif i == 4 then
            set statColor = "|c00DC143C" // Hitpoints
        elseif i == 7 then
            set statColor = "|c001E90FF" // Mana
        elseif i == 10 then
            set statColor = "|c00FFFF80" // Critical Chance
        elseif i == 35 then
            set statColor = "|c00FFFF80" // Block Chance
        elseif i == 36 then
            set statColor = "|c00FFFF80" // Hit Chance
        elseif i == 37 then
            set statColor = "|c00FFAA82" // Spell Power
        else
            set statColor = "|c00FFFFFF" // Default white
        endif

        // Check if the stat should not be multiplied by 100
        if DisplayAsPercent[i] then
            if i == 10 or i == 27 or i == 35 or i == 36 or i == 37 then
                // These stats are percentages but not multiplied by 100
                set sgranted = sgranted + "|n" + "+" + R2Dec2S(amount) + "% " + statColor + DEqStatNames[i] + "|r"
            else
                // Default percentage stats
                set sgranted = sgranted + "|n" + "+" + R2Dec2S(amount * 100) + "%% " + statColor + DEqStatNames[i] + "|r"
            endif
        else
        ...
        /* Original way here, Valdemar change above
...
// Check if ability name contains "EQUIP"
if StringCase(GetObjectName(ia[i]), false) != null and SubString(StringCase(GetObjectName(ia[i]), false), 0, 5) != "EQUIP" then
    set sabgranted = sabgranted + "|n" + GetObjectName(ia[i]) + " " + I2S(R2I(amount))
endif
//set sabgranted = sabgranted + "|n" + GetObjectName(ia[i]) + " " + I2S(R2I(amount)) - original
...
// This will remove items of a specific type from a unit's inventory, until the specified amount of charges is removed
function RemoveDInvItemChargesByType takes unit u, integer itemTypeId, integer amountToRemove returns nothing
    ...
    local integer vanillaInvSize = UnitInventorySize(u)
    local item it
    local integer charges
    local integer currentItemType
...
set amount = amount * (1+(ilvl-1)*DEqIlvlStatX)
    ...
    //do nothing
        ...
        // Evasion / Dodge chance
            set udg_Stats_Dodge[GetUnitUserData(u)] = udg_Stats_Dodge[GetUnitUserData(u)] - R2I(amount)
            /* EVASION original Disabled
        ...
        //Crit Chance
            set udg_Stats_Crit[GetUnitUserData(u)] = udg_Stats_Crit[GetUnitUserData(u)] - R2I(amount)
            /*/ CRIT original ability way - disabled
        ...
        elseif statid == 35 then
            set udg_Stats_Block[GetUnitUserData(u)] = udg_Stats_Block[GetUnitUserData(u)] - R2I(amount)
        //Hit chance
        elseif statid == 36 then
            set udg_Stats_Hit[GetUnitUserData(u)] = udg_Stats_Hit[GetUnitUserData(u)] - R2I(amount)
        // Spell power Pct
        elseif statid == 37 then
            set udg_Stats_SpellPowerPct[GetUnitUserData(u)] = udg_Stats_SpellPowerPct[GetUnitUserData(u)] - R2I(amount)
        // Spell power Flat
        elseif statid == 38 then
            set udg_Stats_SpellPowerFlat[GetUnitUserData(u)] = udg_Stats_SpellPowerFlat[GetUnitUserData(u)] - R2I(amount)
...
set amount = amount * (1+(ilvl-1)*DEqIlvlStatX)
    ...
    //do nothing
        ...
        // Evasion / Dodge
            set udg_Stats_Dodge[GetUnitUserData(u)] = udg_Stats_Dodge[GetUnitUserData(u)] + R2I(amount)
            /* ORIGINAL EVASION ability based code - disabled for now
            if GetUnitAbilityLevel(u, 'DQEV') < 1 then
            call UnitAddAbility(u, 'DQEV')
            call BlzUnitDisableAbility(u, 'DQEV', FALSE, TRUE)
            endif
        ...
        //Crit Chance
            set udg_Stats_Crit[GetUnitUserData(u)] = udg_Stats_Crit[GetUnitUserData(u)] + R2I(amount)
            /* ORIGINAL CRIT ability based code - disabled for now
            if GetUnitAbilityLevel(u, 'DQCS') < 1 then
            call UnitAddAbility(u, 'DQCS')
            call BlzUnitDisableAbility(u, 'DQCS', FALSE, TRUE)
            set a = BlzGetUnitAbility(u, 'DQCS')
...
local integer add
local integer max
local integer newTotal

// Infinite Stacking system is disabled because stacking utilizes now item level as max stack size
//if InfiniteStackingSystemAllowed == TRUE then
if DInventoryIsItemStackable(it) == TRUE then
    if PlayerStackingMode[pid] == 2 then
        // Limited stacking using item level as max stack size
        set targetSlot = FirstStackableItemSlotOfBID(it, pid, bid)
        if targetSlot > -1 then
            set stackItem = DInventoryDB[bid].item[targetSlot]
            set current = GetItemCharges(stackItem)
            set add = GetItemCharges(it)
            set max = GetItemLevel(stackItem) // max stack size
            set newTotal = current + add

            if newTotal > max then
                // Fill stack to max
                call SetItemCharges(stackItem, max)
                // Leave remainder in incoming item
                call SetItemCharges(it, newTotal - max)

                // Update filled stack UI
                if pid == GetPlayerId(GetLocalPlayer()) then
                    if BlzFrameIsVisible(InventoryLowestFrame[pid]) == TRUE then
                        if DItemSlotId2SlotFrameId(pid, targetSlot) != -1 and bid == CurrentBID[pid] then
                ...
                // All charges fit -> merge and remove new item
                call SetItemCharges(stackItem, newTotal)
                call RemoveItem(it)
                set it = null

                // Update merged stack UI
                if pid == GetPlayerId(GetLocalPlayer()) then
                    if BlzFrameIsVisible(InventoryLowestFrame[pid]) == TRUE then
                        if DItemSlotId2SlotFrameId(pid, targetSlot) != -1 and bid == CurrentBID[pid] then
                            call DInvSlotDataIntoFrame(pid, bid, targetSlot, DItemSlotId2SlotFrameId(pid, targetSlot))
            ...
            // No existing stack -> handled later
        endif
    endif
endif
//endif
...
if it == null then
    ...
    if targetSlot != -1 then
        ...
        set DInvItemHandleDB[ihndl].integer[2] = targetSlot  // CRITICAL FIX: Store slot ID
...
if pid == GetPlayerId(GetLocalPlayer()) then
    if BlzFrameIsVisible(InventoryLowestFrame[pid]) == TRUE then
        if DItemSlotId2SlotFrameId(pid, targetSlot) >= 0 and bid == CurrentBID[pid] then
            call DInvSlotDataIntoFrame(pid, bid, targetSlot, DItemSlotId2SlotFrameId(pid, targetSlot))
        endif
    endif
endif
...
*/
        ...
        set DInvItemHandleDB[ihndl].integer[0] = pid + 1
        set DInvItemHandleDB[ihndl].integer[1] = bid
        set DInvItemHandleDB[ihndl].integer[2] = targetSlot  // CRITICAL FIX: Store slot ID in old function too
        set DInvItemHandleDB[ihndl].integer[6] = eqid
        // set trove ID
...
// CRITICAL FIX: Store item handle ID BEFORE removing item from inventory
...
function GetDInvItemChargesByType takes unit u, integer itemTypeId returns integer
    ...
    // First, check DInventory
    loop
        exitwhen slotId >= maxCapacity // Iterate through all slots in the custom inventory
        set it = DInventoryDB[bid].item[slotId]
        if it != null and GetItemTypeId(it) == itemTypeId then
            set charges = GetItemCharges(it)
            // CRITICAL FIX Oct 22 2025: Count items with 0 charges as 1 item
            // Non-stackable items (quest items, equipment) have 0 charges but still count as 1 item
            if charges <= 0 then
                set totalCharges = totalCharges + 1
            else
    ...
    // CRITICAL FIX Feb 15 2026: Also check vanilla inventory
    // Quest items can be in either DInventory OR vanilla inventory
    loop
...
// Also checks vanilla inventory after checking DInventory
function GetDInvItemChargesByTypeThreshold takes unit u, integer itemTypeId, integer threshold returns boolean
    ...
    // NOTE: Do NOT use TriggerSleepAction inside this function - it breaks return values!
    // If you need a delay for the inventory to update, call TriggerSleepAction BEFORE calling this function
    ...
    // Check vanilla inventory as well
    loop
        exitwhen vanillaSlot >= vanillaInvSize
        set it = UnitItemInSlot(u, vanillaSlot)
        if it != null and not IsItemStoredInDInv(it) and GetItemTypeId(it) == itemTypeId then
