...
globals
    ...
    public framehandle ReturnButton
...
private function ReturnAction takes nothing returns nothing
    call Hide()
    call MasterUI_Show()
endfunction
...
local integer i
    ...
    set ReturnButton = BlzCreateFrameByType("GLUETEXTBUTTON", "TasQuestBoxReturnButton", Parent, "ScriptDialogButton", 0)
    call BlzFrameSetSize(ReturnButton, 0.065, 0.03)
    call BlzFrameSetText(ReturnButton, "Return")
    call BlzFrameSetPoint(ReturnButton, FRAMEPOINT_TOPRIGHT, BlzGetFrameByName("TasQuestBoxCloseButton1", 0), FRAMEPOINT_TOPLEFT, -0.008, 0.0)
    call BlzTriggerRegisterFrameEvent(ReturnTrigger, ReturnButton, FRAMEEVENT_CONTROL_CLICK)
    call BlzTriggerRegisterFrameEvent(ClearFocusTrigger, ReturnButton, FRAMEEVENT_CONTROL_CLICK)
    ...
    set ReturnTrigger = CreateTrigger()
    call TriggerAddAction(ReturnTrigger, function ReturnAction)
