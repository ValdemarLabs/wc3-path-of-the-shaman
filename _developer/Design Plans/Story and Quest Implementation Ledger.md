# Story and Quest Implementation Ledger
...
- [1. Purpose](#1-purpose)
- [2. Authority, evidence, and status](#2-authority-evidence-and-status)
- [3. Canonical world and naming baseline](#3-canonical-world-and-naming-baseline)
- [4. Zone progression and narrative use](#4-zone-progression-and-narrative-use)
- [5. Current implementation ledger](#5-current-implementation-ledger)
- [6. Story pillars and player role](#6-story-pillars-and-player-role)
- [7. Proposed coherent story arc](#7-proposed-coherent-story-arc)
- [8. Class and companion arcs](#8-class-and-companion-arcs)
- [9. Planned generic and regional quest bank](#9-planned-generic-and-regional-quest-bank)
- [10. Dungeon quest packages](#10-dungeon-quest-packages)
- [11. Quest-giver and event contracts](#11-quest-giver-and-event-contracts)
- [12. Legacy content requiring recovery](#12-legacy-content-requiring-recovery)
- [13. Open and resolved canon and implementation decisions](#13-open-and-resolved-canon-and-implementation-decisions)
- [14. Recommended implementation order](#14-recommended-implementation-order)
- [15. Quest design and implementation checklist](#15-quest-design-and-implementation-checklist)
- [16. Maintenance rules](#16-maintenance-rules)

## 1. Purpose

This file is the shared technical source of truth for implementing the intended *Path of the Shaman* story and quest structure. Read it together with [Story and Quest Design.md](Story%20and%20Quest%20Design.md) before creating or materially changing a quest, quest giver, story dialogue, dungeon quest, or story-driven world event.

The ledger does not replace the map, current JASS, or recovered GUI triggers. It connects those sources and records which ideas are current, incomplete, legacy, proposed, or still need World Editor verification. When implementation reveals better information, update this ledger and, when the discovery changes narrative canon, update the narrative design file as well.
...
- Faction metadata identifies quest affiliation and rewards; reputation availability gates are opt-in through an explicit required-reputation setting.
...
| `qZulkis.j` | Meet with Chieftain Thork; Rescue the Brother | **Implemented prologue JASS; runtime validation pending.** `qZulkis_StartPrologue()` owns the six-camera river arrival, temporary `'odes'` ship, living Darkspear shore party, Thork meeting, off-screen corpse/captive staging immediately after Thork's return order, wounded-witch-doctor testimony and interrupted death vocal, the chance arrival of a four-grunt orc patrol, Zul'karak relocation through `udg_Zulkarak`, rescue in Bramblehide Village `701`, Horde-base staging, and fade back to Nazgrek. Each scripted-camera entry invokes the reset-safe preparation path once, stopping inherited gameplay target control, smoothing, and queued reset motion before the scene takes ownership; CameraControl also suspends DynamicMinimap and releases its last chunk bounds for the full scripted-camera session, then restores its previous enabled state when gameplay camera control resumes. The later literal setup pairs remain adjacent and unchanged inside that already-suspended scene: the arrival snaps to `IntroZulkisCam2` before its timed movement toward camera 1, then cuts to camera 5 and moves toward camera 6, while the landing snaps to camera 3 before its timed movement toward camera 4. Darkspear corpses use the recovered GUI permanent-flesh staging throughout Zul'kis gameplay, while the wounded witch doctor remains frozen in his lying pose after a sound-only fake death; all six resume normal decay when the prologue ends. The patrol starts approaching as soon as that fake death occurs, joins as four temporary companion grunts for Rescue the Brother, bypasses the ordinary low-level companion cap for this scripted section, and is unregistered and removed when Zul'kis's prologue ends. Rescue completion requires the nearby hostile forest trolls around Zul'karak to be dead and uses `IntroZulkisCam8` to camera 9 over 30 seconds. When his playable section begins, `Start.j` equips Zul'kis with Shadowcaster's Scepter and a mostly common, deliberately weak Darkspear travel set, plus two Mana Potions, one Healing Potion, and Purified Water in his native inventory. Graveyard `2` (`Graveyard02`) temporarily replaces Nazgrek's gameplay selection, and prologue completion explicitly selects Nazgrek's initial Graveyard `4` (`Graveyard04`). The transition waits for full black before hiding, pausing, and neutralizing Nazgrek or hiding Shadowclaw; any Nazgrek-focused companions are silently removed from the active and focus groups, hidden, and suspended for Zul'kis gameplay, then restored with their prior visibility, pause, suspension, focus, and UnitHider state only after Nazgrek's return decision sequence ends. The intro ship is created and ordered toward its waypoint before the black mask fades in. Prologue completion then explicitly discovers Call of the Horde as control returns to Nazgrek. Zul'karak's later quest-giver quests, recruit unlock, and dedicated combat/home AI remain planned separately. |
| `qRagno.j` | Protect the Outpost; Gnoll Headcount; Lumberjack Duties; Kobold Thieves; Satyr Negotiations; Call of the Horde | **Implemented JASS.** Protect the Outpost is externally started and auto-completed by its scripted defense. Its intro plays the `OrcGrunt_0012` attack warning, waits four seconds, and then continues with the `OrcGrunt_0013` overwhelmed response and Nazgrek's reply; its independent camera shots include the same delay. Active gnolls left idle or inside a wave spawn region are periodically ordered to attack the Horde mountain outpost. The intro restores CameraControl to the player's pre-cinematic camera unit on both normal completion and ESC skip. Quest acceptance occurs when the gnoll-attack intro cinematic ends, so the shared five-second discovery delay presents the quest after the scene instead of interrupting it. Delayed initialization also starts the encounter when an owned hero is already inside one of its entry regions, preventing its completion-gated follow-up quests from remaining unavailable after a missed region-enter event. If the encounter interrupts a Ragno interaction, it first skips the active sequence and closes its menu so no pending-dialog state survives into the cinematic. Gnoll Headcount, Lumberjack Duties, Kobold Thieves, and Satyr Negotiations all require its completion and become available afterward regardless of Horde reputation; Protect the Outpost completion is their complete access gate. Lumberjack Duties binds its peon to the hero who accepted the quest, temporarily removes the peon from FollowSystem while harvesting a nearby quest tree, and restores following after the harvest cycle. If Ragno is replaced, registering the new handle cannot overwrite the old type owner before transfer; pre-registered empty replacements can still receive the quests, and unique-title lookup repairs a missed handle transfer before choices are built. Available-but-inactive discovery state is normalized so it cannot produce a yellow marker with an empty menu. Ragno's entry, greeting, spoken sequences, and visible-menu exit use the shared DialogInteraction/DialogSystem ESC lifecycle: one end-cinematic dispatch completes pending entry and greeting through to choices, skips active speech through its finish callback, or exits visible choices after the same fullscreen event has settled. Ragno does not install a competing local menu escape action. Its legacy `QUEST_MOUNTAIN_DEFENSE` alias still points to Protect the Outpost internally. Granis owns the separate QuestData for the later Mountain Defense, but Ragno remains that quest's field commander, encounter anchor, required survivor, and principal battlefield speaker. Satyr Negotiations reaches Zaekolaerr; its arena outcome now waits for a successful Coliseum challenge started through satyr arena master `n62V`, while the escape, betrayal, and trust follow-ups remain partial. Call of the Horde requires Protect the Outpost, Zul'kis prologue completion, and an external unlock from the finished Nazgrek return sequence; it is unit-specific to Nazgrek so its giver/receiver markers stay inactive while Nazgrek is neither owned nor a companion. Protect the Outpost stages active Shadowclaw beside Nazgrek for its completion and preserves that position through the Zul'kis prologue before returning to Nazgrek gameplay. Its completion suspends and clears gameplay-camera ownership, explicitly avoids ending cinematic control when it hands the still-black screen directly to `qZulkis_StartPrologue()`, does not schedule the letter discovery during that handoff, and resumes Nazgrek only in the fallback path where Zul'kis's prologue was already complete. It also unlocks the one-shot southern mountain-camp grunt exchange through `HordeUnitsRandomChat_EnableMountainChat()`. |
...
| `qGraknar.j` | Mistaken Kin | **Implemented JASS.** Level 2 Normal side quest that spawns Kodo `o008` at `gg_rct_KodoSpawn`, finds it at 500 range, escorts it through `FollowSystem`, and returns it to Graknar before turn-in. Mistaken Kin has no reputation gate; its Horde faction metadata must not silently require Neutral standing. The shared unique-title recovery repairs a missed Graknar handle transfer before building the menu, and orphaned available/discovered state cannot hide the accept choice. Graknar's configured interaction inherits the shared ESC lifecycle: entry fades advance to the greeting, spoken sequences advance to their completion callback, visible choices close cleanly, and exit fades restore play immediately; one physical ESC is consumed once even though Warcraft raises both its key and cinematic events. Graknar's Trade option reasserts his dedicated bag catalog before opening the shop. ShopUI forces its initial merchant repaint after the originating dialog event finishes, and Back defers rebuilding Graknar's current quest/trade choices until the shop frame event has finished; the returned choices restore their ESC cleanup instead of leaving cinematic control active. The quest's zone and canonical `o61S` placement still require WE verification; every other bag merchant currently using `o61S` needs a distinct unit rawcode and identity. |
...
| `qANightToRemember.j` | A Night To Remember | **Implemented JASS.** Repeatable, zone-aware social quest with three witnesses and randomized make-amends tasks. When the active party contains an eligible AI hero companion, at least one witness objective is assigned to a companion. Satyr witnesses cannot roll Satyr or Shadowdancer kill targets; their kill pool is limited to Gnolls and Dark Trolls. The recorded amends requests remain intentionally broad until witness-specific task templates and dialogue are approved together. It is an optional character vignette, not a canonical story requirement. |
...
Ten one-time vendor escorts are implemented through `FollowSystem`. The original Nara Stormhoof, Cedran Pike, and Selyth Venomcup routes are joined by three Orc escorts (Kargun Ashblade, Rukgar Longroad, and Gorthak Jungle Banner), two Goblin escorts (Giznak Edgeprice and Rixit Roadcoin), one Bonecrusher Ogre escort (Mugrok Ironclub), and one Morgrim Dwarf escort (Torren Deepsteel). Rukgar, Gorthak, Rixit, and Mugrok require protection to a named vendor and back to their recorded start; the other six finish at their destination.

The merchant is temporarily invulnerable during the route so a dead shop unit cannot strand a generic quest without a valid giver; prior invulnerability is restored when following ends, and an abandoned route returns the merchant to its recorded start. Five routes spawn one tracked, quest-owned hostile group at configured outbound or return progress and order it to attack the escort; surviving spawned units are cleaned up on quest exit. Nara, Cedran, Kargun, Giznak, and Torren unlock trading only after their escort is turned in. The shared escort path includes giver-specific travel/turnaround/ambush dialogue and an optional nearby AI-companion response. In-progress conversations use three vendor-specific lines and three activity-specific replies apiece for Nazgrek and Zul'kis instead of the race-neutral `GenericQuest` escort pool. Exact start placement, destination placement, walkability, ambush pathing, meaningful route distance, and Rixit/Snikka placement remain **Verify in WE**.
...
| Mad Blix | `BossMadBlix.j` contains the recoverable mana-absorption mechanic, reports `BOSS_EVENT_DEATH` to `qBoomBrothers_ReportMadBlixDefeated`, and records Boom Mine as encounter context without joining its respawn roster; legacy phase files are empty. | Quest completion is wired and Mad Blix remains dead after his one-time story defeat, but the boss remains **Partial** until additional mechanics are deliberately designed and tested. |
...
> **Implementation mirror:** This section and the act tables below retain technical mapping, design IDs, and per-quest status. [Story and Quest Design.md](Story%20and%20Quest%20Design.md) owns the current narrative treatment. If the two files conflict about story meaning, resolve the discrepancy there and then bring this ledger's implementation mapping into alignment.
...
| ST-P-06 | Protect the Outpost | **Implemented JASS** | First visible act of service. The gnoll-attack cinematic completes before quest acceptance, leaving the shared five-second discovery delay to present the objective after the scene. Ragno survives and notices Nazgrek, but Call of the Horde remains undiscovered through Zul'kis's parallel intro. Protect the Outpost completion ends the separate Nazgrek section with a black-screen handoff to Zul'kis. |
...
| Design ID | Quest / beat | Status | Purpose and connection |
|---|---|---|---|
| ST-ZP-01 | Darkspear landing cinematic | **Implemented JASS; runtime validation pending** | `qZulkis` takes over behind a controlled black-screen handoff and waits until the fade is fully black before hiding Nazgrek, Shadowclaw, or Nazgrek's companions. Those companions are silently suspended from the synchronized active roster and focus groups, paused, and hidden while retaining their controller registration, leader, mode, icon, and prior state for restoration. Because their GUI roster entries are temporarily removed, they neither appear in Zul'kis's companion count nor consume his party capacity, and roster-repair passes cannot reactivate them. The handoff initializes Zul'kis's inventory/equipment and grants his one-time starter loadout through `Start.j`, temporarily selects Graveyard02 ID `2` through `Revival` so its active marker is shown, and prepares scripted-camera ownership once before the fade. This preparation suspends gameplay tracking, stops the zero-duration reset queued while releasing its target controller, disables smoothing, suspends DynamicMinimap updates, and restores full-map camera bounds so the previous chunk cannot constrain a distant hard cut. The two literal GUI/trailer calls then remain adjacent and own subsequent cuts inside that already-suspended cinematic: the river applies `IntroZulkisCam2` at `0.00` seconds and camera 1 over 20 seconds; five seconds later it applies camera 5 at `0.00` seconds and camera 6 over 15 seconds; the shore dialogue applies camera 3 at `0.00` seconds and camera 4 over 20 seconds; and the Nazgrek return applies `NazgrekDecision1` at `0.00` seconds before its timed move to camera 2. Later gameplay-to-cinematic entries for the broken landing and rescue perform their own single preparation because gameplay camera tracking and DynamicMinimap's prior enabled state are restored between scenes. Nazgrek's initial Graveyard04 ID `4` and its marker are selected before the return scene. The loadout uses Shadowcaster's Scepter, eight mostly flavor-only common Darkspear travel pieces, two Mana Potions, one Healing Potion, and Purified Water. It creates ship `'odes'` at `gg_rct_ZulkisShipWP1`, issues its movement order toward `gg_rct_ZulkisShipWP2` while the screen remains black, then applies the opening camera and starts the fade-in; the ship is removed on arrival. It stages Zul'kis plus six Neutral Passive, purple-colored Darkspear trolls on the confirmed shore rects. Two additional Neutral Passive, purple-colored Darkspear headhunters join Zul'kis through the companion controller when initial gameplay begins and are removed with the other temporary prologue companions at convergence. ESC skips the non-dialogue ship movement into the shore scene, and the transition state prevents repeat fade scheduling. |
| ST-ZP-02 | Brothers on the shore | **Implemented dialogue** | Two minimal narrator lines begin with the river-arrival cinematic without a default speaker, active NPC, or transmission unit. DialogSystem renders them as named `Narrator` cinematic transmissions with a null unit and null transmission sound handle, while ExSound separately supplies the voice playback, `udg_ExSoundString`, and duration. They establish that Zul'kis and his Darkspear tribe are arriving along Havenwoods' eastern river after answering Thork's call to aid the orcish clan, and identify Zul'karak as Zul'kis's elder brother. The later shore sequence begins directly with the brothers and leaves all tension and foreshadowing to the characters. Zul'karak warns that the tribe's wise one sensed a dark threat, Zul'kis trusts Thork's promise of a quick meeting, and Zul'karak remains to guard the landing. Speaker-owned constants live in `Voicelines_Narrator.j`, `Voicelines_Zulkis.j`, and `Voicelines_Zulkarak.j`. |
| ST-ZP-03 | Meet with Chieftain Thork | **Implemented Normal + Story** | The first `qZulkis` quest begins after the shore conversation. Thork selection is redirected from `qChieftainThork` while the prologue is active. Zul'kis first hails the chieftain and identifies himself by name; Thork then questions the tribe's absence and orders Zul'kis back while warning about Havenwoods humans. The same quest tracks the return to shore. |
| ST-ZP-04 | The broken landing | **Implemented continuation; runtime validation pending** | Immediately after Thork orders Zul'kis back, the unseen shore swaps the five headhunters to Neutral Passive, purple-colored permanent fleshy corpses through the recovered GUI staging helper, moves `udg_Zulkarak` to `gg_rct_ZulkarakCaptive`, damages the placed `'odes'` frigate inside `gg_rct_HavenwoodsShip` to 35% life, and places the witch doctor at 03 in a slowed lying pose with intermittent chest blood effects. Returning near `gg_rct_ZulkisStart` reveals that already-completed staging. Zul'kis looks between the bodies in shock, calls for his brother, hears the wounded witch doctor, moves beside him, turns toward him without issuing any facing or look-at command to the wounded unit, and demands to know what happened before the testimony begins. The survivor's final line is cut short by stopping the voice, stopping the blood effect, freezing the existing lying pose, and playing the native Witch Doctor death sound; the unit is not killed and its death animation is not restarted. That callback immediately cancels the remaining stopped-line timer and begins monitoring the already-staged patrol, so the scene does not sit silent before the grunts approach. When the prologue ends under black, the fake body is replaced directly with a normally decaying permanent corpse. Four orc grunts created off-camera inside `gg_rct_HavenwoodsOrcPatrol` enter the same uninterrupted cinematic and pretend they only heard the shoreline battle. Once the lead grunt reaches Zul'kis, all four settle into their exact 400-range formation, stop, face him, and the camera hard-cuts to `IntroZulkisCam7` before anyone speaks. They offer to help Zul'kis fight through the forest trolls while he supports them. The five actual Darkspear corpses and the witch doctor's frozen fake body remain preserved during Zul'kis gameplay; all six use normal corpse decay after the prologue. |
| ST-ZP-05 | Rescue the Brother | **Implemented Normal + Story** | Rescue the existing `udg_Zulkarak` (`n65F`) in Bloodtusk-controlled Bramblehide Village `701`. No artificial enemy guard wave is created; the village's existing forest trolls supply the danger. The four-grunt patrol joins Zul'kis through the companion controller in Normal mode, temporarily exceeds the ordinary level-based party cap, accepts the shared group modes and companion Move/Attack orders, and is removed completely after the prologue. While at least one patrol grunt remains alive in the companion group, nearby visible forest trolls occasionally accuse the unexpected orcs of attacking their village and react to the Darkspear-orc group without naming Thork or revealing the Horde arrangement. A dedicated hint recommends keeping Zul'kis behind the grunts to heal and support them. Approaching Zul'karak only starts completion after no living hostile forest troll remains within 700 range. The completion scene hard-cuts to `IntroZulkisCam8` and moves to camera 9 over 30 seconds, then opens with the brothers recognizing each other; Zul'karak gravely describes an exterminatory attack rather than dismissing it as an ordinary territorial rivalry, says his captors questioned what he witnessed, and remains unable to identify who arranged the false flag. During the resulting black screen, Zul'kis and Zul'karak are staged at the Horde base, Nazgrek is moved to `gg_rct_NazgrekOnHill`, and restored Shadowclaw is placed beside him. The reveal slowly pans from `NazgrekDecision1` to `NazgrekDecision2` while Nazgrek tells `udg_Ragno` that he has decided to help only with Thork's urgent mission, voices his clouded elemental unease, and resolves to refresh the shamanic practices dulled during exile. Nazgrek faces Ragno while occasionally glancing at Shadowclaw; normal Nazgrek gameplay, the saved Nazgrek companion roster, and Call of the Horde discovery resume only after this decision scene ends. |
| ST-ZP-06 | Call of the Horde convergence | **Implemented integration; runtime validation pending** | `qRagno` starts the Zul'kis prologue from Protect the Outpost's completion fade without discovering Call of the Horde. When Nazgrek's return decision cinematic finishes, `qZulkis` marks the prologue complete and invokes Ragno's one-shot discovery/letter handoff. `qChieftainThork` gates letter readiness/completion on `qZulkis_IsPrologueCompleted()` and only then restores Zul'kis as a visible, vulnerable Player 1 companion during the existing Nazgrek meeting. |

Hidden story truth: immediately before the broken-landing reveal, Thork sent the same four-grunt patrol as intermediaries to hire the local forest trolls to destroy the Darkspear landing and leave no survivors before Zul'kis returned. The attack was staged to implicate humans, binding a grieving Zul'kis to Thork's Horde through a manufactured common enemy. The patrol returns from that assignment, pretends it only heard the battle, and volunteers as Zul'kis's escort to preserve the deception and control what he discovers. Zul'karak's survival and capture were not part of Thork's plan, nor was Zul'kis meant to find him; the wounded witch doctor's brief survival was another failure in the cleanup. The captors question Zul'karak about what he saw because they need to know whether the false flag was exposed, but Zul'karak cannot identify the shore attackers. Do not reveal Thork's role during the prologue. Seed recoverable inconsistencies—human-looking weapon marks, the attack timing, missing supplies, forest-troll payment, or survivor testimony—for the later investigation.
...
Use the witch doctor at `gg_rct_CorpseTroll03` as the dying survivor, so every visible arrival troll has a return-scene outcome. Preserve his existing lying pose through the testimony and fake-death beat; do not call `KillUnit` or restart the death animation in view. Convert that fake body to the sixth permanent corpse only after the prologue fades to black. Move or hide `udg_Zulkarak` when the landing is destroyed, then relocate that same unit to captivity; never leave a shore copy behind, search for another `n65F`, or create a second brother for the rescue.

#### Act I-C — Convergence and earning a place (levels 3–10)

| Design ID | Quest / beat | Status | Purpose and connection |
|---|---|---|---|
| ST-A1-01 | Call of the Horde | **Implemented JASS with Zul'kis gate** | The quest is discovered and Ragno's blood-signed letter is granted only when Zul'kis's prologue and Nazgrek's return decision cinematic have both finished. The player then carries it to Chieftain Thork. Its completion is the first Nazgrek/Zul'kis meeting and the point where shared gameplay begins; `qChieftainThork` requires `qZulkis_IsPrologueCompleted()` before readiness, completion, or enabling Zul'kis. |
...
5. **Boom Will Be Back** — defeat the registered Mad Blix boss in Boom Mine `104`, permanently remove him from that session, return to the Boom Brothers, receive Crown of Kings +5 `ckng`, and set the mine-reclaimed/access state. Mad Blix is not part of the dungeon's 30-minute respawn roster.
