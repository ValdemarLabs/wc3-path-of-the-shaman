...
library HealEngine
    globals 
        ...
        */
        ...
        private constant real HEAL_CHECK_INTERVAL       = 0.10     // Reduced from 0.05 to 0.10 for better performance
        ...
        private constant boolean FIRE_REGEN_EVENT       = false     // If false, regeneration will not fire AfterHealEvent (no floating text)
        ...
        */
        ...
        private constant real MAX_PREHEAL_EVENTS        = 2.00      // PreHealEvent will fire this many time. Useful if you want to make sure modifiers are applierd if the right order (% then flat for example).
        ...
        private constant integer MAX_HEALS_PER_FRAME    = 25        // Limit heals processed per frame to prevent lag spikes
    endglobals 
    ...
    */
    ...
    struct HealDetection
        ...
        private static method CheckLoop takes nothing returns nothing
            ...
            local real diff
            local real heal
            local real regenValue
            
            loop
                exitwhen i > max
                
                set unitIndex = indices[i]
                ...
                // Replace RMaxBJ with inline comparison for performance
                if regen[unitIndex] > 0.00 then
                    set regenValue = regen[unitIndex]
                else
                    set regenValue = 0.00
                endif
                set heal                = diff - regenValue
                ...
                if heal > HEAL_THRESHOLD then
                    ...
                    if regenTimeLeft[unitIndex] <= 0.00 then
                        ...
                        if diff > REGEN_THRESHOLD and FIRE_REGEN_EVENT then
                             // Set Up variables
                            set udg_HealAmount = heal
                            set udg_HealTarget = target
        ...
        private static method onInit takes nothing returns nothing
            if USE_HEAL_DETECTION then 
                ...
                call TimerStart(healTimer, HEAL_CHECK_INTERVAL, true, null)
    ...
    struct Heal
        ...
        */
        private static method Run takes nothing returns nothing
            ...
            local real i = 1.00
            local integer healsProcessed = 0

            loop 
                exitwhen first == null or healsProcessed >= MAX_HEALS_PER_FRAME
                ...
                set healsProcessed = healsProcessed + 1
                set i = 1.00  // Reset for next heal's PreHealEvents
            endloop
            
            // If there are still heals to process, schedule them for next frame
            if first != null then
                call TimerStart(CreateTimer(), 0.00, false, function Heal.Run)
            endif
