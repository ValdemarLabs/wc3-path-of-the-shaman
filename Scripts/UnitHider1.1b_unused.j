...
globals
    ...
    group tempGroup = CreateGroup() // Stores hidden units
...
function UnitHider_IsUnitNearReferenceUnits takes unit u returns boolean
    ...
    call GroupAddGroup(udg_UnitHider_ReferenceGroup, tempGroup) // Clear and copy reference group to temp group

    set ref = FirstOfGroup(tempGroup)
    loop
        exitwhen ref == null
        ...
        call GroupRemoveUnit(tempGroup, ref)
        set ref = FirstOfGroup(tempGroup)
    endloop
...
function UnitHider_HideDistantUnits takes nothing returns nothing
    ...
    // Unhide units that are near reference units
    loop
        ...
        if UnitHider_IsUnitNearReferenceUnits(u) then
            ...
            call GroupRemoveUnit(hiddenUnits, u) // safely removes current unit from hiddenUnits
            ...
            // only advance if not removed, otherwise we already popped it
            call GroupRemoveUnit(hiddenUnits, u)
            call GroupAddUnit(hiddenUnits, u) // re-add so we can advance safely
        endif
    endloop

    // Hide units that are not near reference units
    call GroupEnumUnitsInRect(hiddenUnits, GetWorldBounds(), null)
    loop
        set u = FirstOfGroup(hiddenUnits)
        exitwhen u == null
