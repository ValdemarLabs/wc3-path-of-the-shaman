library QuestIconSystem requires SpeciFX
//===========================================================================
/*
    QuestIconSystem 1.1
    ...
    For automatic quest availability evaluation based on level/reputation/events, see QuestEvaluationSystem.j
...
//===========================================================================
globals
    hashtable QUEST_ICON_TABLE = InitHashtable() // Public for QuestEvaluationSystem
    private hashtable QUEST_BACKUP_TABLE = InitHashtable() // Backup by unit-type for respawn
    ...
    // ---- per-unit quest list key offsets ---- (Public for QuestEvaluationSystem)
...
function QuestIcon_RefreshIcon takes unit u, integer questID, string questType, integer questState returns nothing
    ...
    //  Icon logic
    // Unavailable (gray exclamation) - show overhead icon but NO minimap marker
    if questState == 1 then
        set model = QUEST_ICON_MODEL_GRAY_EXCLAMATION
        set pingStyle = -1 // No minimap icon for unavailable quests
...
//===========================================================================
// BACKUP QUEST DATA (for respawn)
// Stores active quest data by unit-type ID so it can be restored on respawn
//===========================================================================
function QuestIcon_BackupQuestData takes unit u returns nothing
    local integer unitTypeId = GetUnitTypeId(u)
    local integer id = GetHandleId(u)
    local integer count = LoadInteger(QUEST_ICON_TABLE, id, NPC_QUEST_COUNT_KEY)
    local integer i = 0
    local integer questID
    local string questType
    local integer questState
    
    if count == 0 then
        return // No quests to backup
    endif
    
    // Clear old backup for this unit type
    call FlushChildHashtable(QUEST_BACKUP_TABLE, unitTypeId)
    
    // Backup quest count
    call SaveInteger(QUEST_BACKUP_TABLE, unitTypeId, NPC_QUEST_COUNT_KEY, count)
    
    // Backup each quest (only active quests - states 3 and 5)
    loop
        exitwhen i >= count
        set questID = LoadInteger(QUEST_ICON_TABLE, id, i*100 + QUEST_ID_KEY)
        set questType = LoadStr(QUEST_ICON_TABLE, id, i*100 + QUEST_TYPE_KEY)
        set questState = LoadInteger(QUEST_ICON_TABLE, id, i*100 + QUEST_STATE_KEY)
        
        call SaveInteger(QUEST_BACKUP_TABLE, unitTypeId, i*100 + QUEST_ID_KEY, questID)
        call SaveStr(QUEST_BACKUP_TABLE, unitTypeId, i*100 + QUEST_TYPE_KEY, questType)
        call SaveInteger(QUEST_BACKUP_TABLE, unitTypeId, i*100 + QUEST_STATE_KEY, questState)
...
//===========================================================================
// RESTORE QUEST DATA (after respawn)
// Restores active quest data from backup using unit-type ID
//===========================================================================
function QuestIcon_RestoreQuestData takes unit u returns nothing
    local integer unitTypeId = GetUnitTypeId(u)
    local integer id = GetHandleId(u)
    local integer count = LoadInteger(QUEST_BACKUP_TABLE, unitTypeId, NPC_QUEST_COUNT_KEY)
    local integer i = 0
    local integer questID
    local string questType
    local integer questState
    ...
    if count == 0 then
        return // No backup data for this unit type
    endif
    
    // Restore quest count
    call SaveInteger(QUEST_ICON_TABLE, id, NPC_QUEST_COUNT_KEY, count)
    
    // Restore each quest
    loop
        exitwhen i >= count
        set questID = LoadInteger(QUEST_BACKUP_TABLE, unitTypeId, i*100 + QUEST_ID_KEY)
        set questType = LoadStr(QUEST_BACKUP_TABLE, unitTypeId, i*100 + QUEST_TYPE_KEY)
        set questState = LoadInteger(QUEST_BACKUP_TABLE, unitTypeId, i*100 + QUEST_STATE_KEY)
        
        call SaveInteger(QUEST_ICON_TABLE, id, i*100 + QUEST_ID_KEY, questID)
        call SaveStr(QUEST_ICON_TABLE, id, i*100 + QUEST_TYPE_KEY, questType)
        call SaveInteger(QUEST_ICON_TABLE, id, i*100 + QUEST_STATE_KEY, questState)
...
//===========================================================================
// DEATH EVENT HANDLER
// Automatically backs up quest data when a quest giver dies
//===========================================================================
private function OnUnitDeath takes nothing returns nothing
    local unit u = GetTriggerUnit()
    local integer count = LoadInteger(QUEST_ICON_TABLE, GetHandleId(u), NPC_QUEST_COUNT_KEY)
    
    // Only backup if this unit has active quests
    if count > 0 then
        call QuestIcon_BackupQuestData(u)
    endif
...
private function Init takes nothing returns nothing
    ...
    // Register death event for all units that might be quest givers
