...
globals
    ...
    // Current-zone presentation gate for interiors and other sheltered zones.
    private timer WeatherPresentationTimer      = CreateTimer()
    private integer WeatherPresentationZoneId   = -1
    private boolean WeatherEffectsSuppressed    = false
...
private function ZoneInheritsParentWeather takes integer zoneId returns boolean
    local ZoneData z = ZonesCore_GetZoneData(zoneId)

    if z == 0 or not z.weatherInheritFromParent then
        return false
    endif
...
private function FindRegionIndex takes rect whichRect returns integer
    ...
    loop
        ...
        if RegionRect[i] == whichRect then
            call Debug("FindRegionIndex: matched rect at index=" + I2S(i))
            return i
        else
            // Print debug info for each comparison
            if RegionRect[i] == null then
                set equalsStr = "null"
            else
                set equalsStr = "not equal"
            endif
            call Debug("FindRegionIndex: RegionRect[" + I2S(i) + "] comparison result: " + equalsStr)
        endif
        set i = i + 1
    endloop
    call Debug("FindRegionIndex: no match found for rect")
    return -1
endfunction
...
private function RainImpactCallback takes nothing returns nothing
    ...
    set showImpact = not WeatherEffectsSuppressed and IsRainImpactVisibleThisStep(intensity)
...
private function RemoveSnowImmediately takes integer regionIndex returns nothing
    if RegionSnowWaveTimer[regionIndex] != null then
        call PauseTimer(RegionSnowWaveTimer[regionIndex])
        call DestroyTimer(RegionSnowWaveTimer[regionIndex])
        set RegionSnowWaveTimer[regionIndex] = null
    ...
    if RegionSnowZoneIndex[regionIndex] != -1 then
        call DestroySnow(RegionSnowZoneIndex[regionIndex])
        set RegionSnowZoneIndex[regionIndex] = -1
    endif
    set RegionSnowWaveCount[regionIndex] = 0
endfunction
    ...
    loop
        ...
        if not showWeather then
            ...
            if weatherType == WEATHER_SNOW_LIGHT or weatherType == WEATHER_SNOW_MEDIUM or weatherType == WEATHER_SNOW_HEAVY then
                call RemoveSnowImmediately(i)
            endif
        else
            if weatherType == WEATHER_RAIN_LIGHT or weatherType == WEATHER_RAIN_MEDIUM or weatherType == WEATHER_RAIN_HEAVY or weatherType == WEATHER_SNOW_LIGHT or weatherType == WEATHER_SNOW_MEDIUM or weatherType == WEATHER_SNOW_HEAVY then
                call EnableSteamBreathInRegion(i)
            endif
            if z != 0 and z.weatherEnableClouds and (weatherType == WEATHER_RAIN_MEDIUM or weatherType == WEATHER_RAIN_HEAVY or weatherType == WEATHER_SNOW_MEDIUM or weatherType == WEATHER_SNOW_HEAVY or weatherType == WEATHER_STORM) then
                call EnableCloudsInRegion(i)
            endif
            if (weatherType == WEATHER_SNOW_LIGHT or weatherType == WEATHER_SNOW_MEDIUM or weatherType == WEATHER_SNOW_HEAVY) and RegionSnowWaveTimer[i] == null then
                call StartSnowInRegion(i, weatherType)
            endif
        endif
        set i = i + 1
    endloop
...
private function RefreshWeatherPresentation takes nothing returns nothing
    ...
    local boolean suppress = false
    local ZoneData z

    if zoneId == WeatherPresentationZoneId then
        return
    endif

    set WeatherPresentationZoneId = zoneId
    set z = ZonesCore_GetZoneData(zoneId)
    if z != 0 then
...
private function StopZoneWeatherInternalEx takes integer zoneIndex, boolean propagateToSubZones returns nothing
    local ZoneData z
    local integer i
    local integer regionIndex
    local integer childZoneId
    local ZoneData childZone
    local rect r
    if zoneIndex <= 0 then
        return
    endif
    ...
    loop
        ...
        set r = z.getWeatherSnowRect(i)
        if r != null then
            call Debug("StopZoneWeatherInternal: Processing snowRect index=" + I2S(i) + " coords minX=" + R2S(GetRectMinX(r)) + ", minY=" + R2S(GetRectMinY(r)) + ", maxX=" + R2S(GetRectMaxX(r)) + ", maxY=" + R2S(GetRectMaxY(r)))
            set regionIndex = FindRegionIndex(r)
            call Debug("StopZoneWeatherInternal: Found regionIndex=" + I2S(regionIndex))
            if regionIndex != -1 then
                call StopRegionWeatherInternal(regionIndex)
            else
                call Debug("StopZoneWeatherInternal: No region found for snowRect index=" + I2S(i))
            endif
        endif
        set i = i + 1
    endloop
    ...
    loop
        ...
        if r != null then
            call Debug("StopZoneWeatherInternal: Processing weatherRect index=" + I2S(i) + " coords minX=" + R2S(GetRectMinX(r)) + ", minY=" + R2S(GetRectMinY(r)) + ", maxX=" + R2S(GetRectMaxX(r)) + ", maxY=" + R2S(GetRectMaxY(r)))
            set regionIndex = FindRegionIndex(r)
            call Debug("StopZoneWeatherInternal: Found regionIndex=" + I2S(regionIndex))
            if regionIndex != -1 then
                call StopRegionWeatherInternal(regionIndex)
            else
                call Debug("StopZoneWeatherInternal: No region found for weatherRect index=" + I2S(i))
            endif
        endif
        set i = i + 1
    endloop
    ...
    if propagateToSubZones then
        set childZoneId = 1
        loop
            exitwhen childZoneId > 2000
            set childZone = ZonesCore_GetZoneData(childZoneId)
            if childZone != 0 and childZone.weatherAllowed and childZone.weatherInheritFromParent and ZonesCore_GetParentZoneId(childZoneId) == zoneIndex then
                call StopZoneWeatherInternalEx(childZoneId, false)
            endif
            set childZoneId = childZoneId + 1
        endloop
    endif
endfunction

private function StopZoneWeatherInternal takes integer zoneIndex returns nothing
    call StopZoneWeatherInternalEx(zoneIndex, true)
endfunction
...
private function SetZoneWeatherInternalEx takes integer zoneIndex, string weatherType, real duration, boolean propagateToSubZones returns nothing
    local ZoneData z
    local integer i
    local rect r
    ...
    if propagateToSubZones then
        ...
        loop
            ...
            set childZone = ZonesCore_GetZoneData(childZoneId)
            if childZone != 0 and childZone.weatherAllowed and childZone.weatherInheritFromParent and ZonesCore_GetParentZoneId(childZoneId) == zoneIndex then
                call SetZoneWeatherInternalEx(childZoneId, weatherType, duration, false)
            endif
            set childZoneId = childZoneId + 1
        endloop
    endif
...
private function SetZoneWeatherInternal takes integer zoneIndex, string weatherType, real duration returns nothing
    call SetZoneWeatherInternalEx(zoneIndex, weatherType, duration, true)
endfunction
...
private function ZoneSeasonalWeatherCheck takes integer zoneIndex returns nothing
    ...
    if ZoneInheritsParentWeather(zoneIndex) then
        return
    endif
...
private function SeasonalWeatherCheck takes nothing returns nothing
    ...
    // Check each valid zone. Inheriting subzones are skipped because their parent drives them.
    loop
        exitwhen zoneId > 2000
        set z = ZonesCore_GetZoneData(zoneId)
        if z != 0 then
...
private function Init takes nothing returns nothing
    ...
    call RefreshWeatherPresentation()
    call TimerStart(WeatherPresentationTimer, 0.25, true, function RefreshWeatherPresentation)
