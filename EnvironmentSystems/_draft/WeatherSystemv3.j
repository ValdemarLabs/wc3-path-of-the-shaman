...
//===========================================================================
globals
        // Per-rect weather state/effect/timer/sound storage using hashtables
        private hashtable WeatherEffectTable = InitHashtable()
        private hashtable WeatherTimerTable = InitHashtable()
        private hashtable WeatherTypeTable = InitHashtable()
        private hashtable WeatherRainSoundTable = InitHashtable()
        private hashtable WeatherAmbientSoundTable = InitHashtable()
    // Debug Mode
...
// Additional per-rect hashtable storage for effects and flags
private hashtable RectRipplesTable = InitHashtable()
private hashtable RectRippleCountTable = InitHashtable()
private hashtable RectHasCloudsTable = InitHashtable()
private hashtable RectHasSteamTable = InitHashtable()
private hashtable RectHasThunderTable = InitHashtable()
private hashtable RectSpawnsSnowTable = InitHashtable()
private hashtable RectSnowZoneIndexTable = InitHashtable()
private hashtable RectSnowWaveTimerTable = InitHashtable()
private hashtable RectSnowWaveCountTable = InitHashtable()
private hashtable RectSnowMaxWavesTable = InitHashtable()
private hashtable RectSnowUnitsPerWaveTable = InitHashtable()

// Helper functions for new per-rect storage
private function SetRectRipples takes rect r, destructable d, integer idx returns nothing
    call SaveWidgetHandle(RectRipplesTable, GetHandleId(r), idx, d)
endfunction
private function GetRectRipples takes rect r, integer idx returns destructable
    return LoadWidgetHandle(RectRipplesTable, GetHandleId(r), idx)
endfunction
private function SetRectRippleCount takes rect r, integer count returns nothing
    call SaveInteger(RectRippleCountTable, GetHandleId(r), 0, count)
endfunction
private function GetRectRippleCount takes rect r returns integer
    return LoadInteger(RectRippleCountTable, GetHandleId(r), 0)
endfunction
private function SetRectHasClouds takes rect r, boolean b returns nothing
    call SaveBoolean(RectHasCloudsTable, GetHandleId(r), 0, b)
endfunction
private function GetRectHasClouds takes rect r returns boolean
    return LoadBoolean(RectHasCloudsTable, GetHandleId(r), 0)
endfunction
private function SetRectHasSteam takes rect r, boolean b returns nothing
    call SaveBoolean(RectHasSteamTable, GetHandleId(r), 0, b)
endfunction
private function GetRectHasSteam takes rect r returns boolean
    return LoadBoolean(RectHasSteamTable, GetHandleId(r), 0)
endfunction
private function SetRectHasThunder takes rect r, boolean b returns nothing
    call SaveBoolean(RectHasThunderTable, GetHandleId(r), 0, b)
endfunction
private function GetRectHasThunder takes rect r returns boolean
    return LoadBoolean(RectHasThunderTable, GetHandleId(r), 0)
endfunction
private function SetRectSpawnsSnow takes rect r, boolean b returns nothing
    call SaveBoolean(RectSpawnsSnowTable, GetHandleId(r), 0, b)
endfunction
private function GetRectSpawnsSnow takes rect r returns boolean
    return LoadBoolean(RectSpawnsSnowTable, GetHandleId(r), 0)
endfunction
private function SetRectSnowZoneIndex takes rect r, integer idx returns nothing
    call SaveInteger(RectSnowZoneIndexTable, GetHandleId(r), 0, idx)
endfunction
private function GetRectSnowZoneIndex takes rect r returns integer
    return LoadInteger(RectSnowZoneIndexTable, GetHandleId(r), 0)
endfunction
private function SetRectSnowWaveTimer takes rect r, timer t returns nothing
    call SaveTimerHandle(RectSnowWaveTimerTable, GetHandleId(r), 0, t)
endfunction
private function GetRectSnowWaveTimer takes rect r returns timer
    return LoadTimerHandle(RectSnowWaveTimerTable, GetHandleId(r), 0)
endfunction
private function SetRectSnowWaveCount takes rect r, integer count returns nothing
    call SaveInteger(RectSnowWaveCountTable, GetHandleId(r), 0, count)
endfunction
private function GetRectSnowWaveCount takes rect r returns integer
    return LoadInteger(RectSnowWaveCountTable, GetHandleId(r), 0)
endfunction
private function SetRectSnowMaxWaves takes rect r, integer count returns nothing
    call SaveInteger(RectSnowMaxWavesTable, GetHandleId(r), 0, count)
endfunction
private function GetRectSnowMaxWaves takes rect r returns integer
    return LoadInteger(RectSnowMaxWavesTable, GetHandleId(r), 0)
endfunction
private function SetRectSnowUnitsPerWave takes rect r, integer count returns nothing
    call SaveInteger(RectSnowUnitsPerWaveTable, GetHandleId(r), 0, count)
endfunction
private function GetRectSnowUnitsPerWave takes rect r returns integer
    return LoadInteger(RectSnowUnitsPerWaveTable, GetHandleId(r), 0)
endfunction

// Helper functions for per-rect storage
private function SetRectWeatherEffect takes rect r, weathereffect w returns nothing
    call SaveWidgetHandle(WeatherEffectTable, GetHandleId(r), 0, w)
endfunction
private function GetRectWeatherEffect takes rect r returns weathereffect
    return LoadWidgetHandle(WeatherEffectTable, GetHandleId(r), 0)
endfunction
private function SetRectWeatherTimer takes rect r, timer t returns nothing
    call SaveTimerHandle(WeatherTimerTable, GetHandleId(r), 0, t)
endfunction
private function GetRectWeatherTimer takes rect r returns timer
    return LoadTimerHandle(WeatherTimerTable, GetHandleId(r), 0)
endfunction
private function SetRectWeatherType takes rect r, string s returns nothing
    call SaveStr(WeatherTypeTable, GetHandleId(r), 0, s)
endfunction
private function GetRectWeatherType takes rect r returns string
    return LoadStr(WeatherTypeTable, GetHandleId(r), 0)
endfunction
private function SetRectRainSound takes rect r, sound s returns nothing
    call SaveSoundHandle(WeatherRainSoundTable, GetHandleId(r), 0, s)
endfunction
private function GetRectRainSound takes rect r returns sound
    return LoadSoundHandle(WeatherRainSoundTable, GetHandleId(r), 0)
endfunction
private function SetRectAmbientSound takes rect r, sound s returns nothing
    call SaveSoundHandle(WeatherAmbientSoundTable, GetHandleId(r), 0, s)
endfunction
private function GetRectAmbientSound takes rect r returns sound
    return LoadSoundHandle(WeatherAmbientSoundTable, GetHandleId(r), 0)
endfunction
...
private function StopRegionWeatherInternal takes integer regionIndex returns nothing
    ...
    local integer zoneIndex = RegionZoneIndex[regionIndex]
    local ZoneData z = ZonesCore_GetZoneData(zoneIndex)
    if regionIndex == -1 then
        return
    endif
    ...
    //debug weather now (should be "none")
    call Debug("Zone weather after stopping region weather: " + ZonesCore_GetZoneData(RegionZoneIndex[regionIndex]).GetWeatherState())
...
private function RegionWeatherExpireCallback takes nothing returns nothing
    ...
    loop
        ...
        if RegionWeatherTimer[i] == t then
            set zoneIndex = RegionZoneIndex[i]
...
private function StartRegionWeatherInternal takes integer regionIndex, string weatherType, real duration returns nothing
    ...
    // Stop existing weather in this region only.
    call StopRegionWeatherInternal(regionIndex)
...
private function Test_SpawnRandomWeatherInZone takes string zoneName returns nothing
    ...
    local ZoneData z
    local string rectList
    local string snowRectList
    ...
    set z = ZonesCore_GetZoneData(zoneId)
    if z == 0 then
        call Debug("Test_SpawnRandomWeatherInZone: ZoneData not found for zone: " + zoneName)
        return
    endif

    // Output all registered weather rects and snow rects for this zone
    set rectList = z.listWeatherRects()
    set snowRectList = z.listWeatherSnowRects()
    call Debug(rectList)
    call Debug(snowRectList)
