...
// Table v6 state is initialized from a struct initializer, matching the
// lifecycle used by the working Reputation system. The library initializer
// never allocates Tables, so a Table initialization failure cannot prevent
// event callback registration.
private function InitializeTableState takes nothing returns nothing
    if tableStateReady then
        return
    endif

    if not exclusionListReady then
        set tableInitStage = 10
...
// Struct initializers run before library initializers under standard JassHelper.
// Table v6 explicitly supports initialization at this stage. If this thread is
// interrupted, InitActions retries the same idempotent initialization at game
// time 0 and tableInitStage identifies the exact step that was reached.
private struct CreepRespawnTableState extends array
    private static method onInit takes nothing returns nothing
        call InitializeTableState()
    endmethod
endstruct
...
private function InitActions takes nothing returns nothing
    ...
    // Retry Table initialization outside the library initializer if the earlier
    // struct-initializer attempt did not complete. This keeps event registration
    // independent from Table allocation.
    if not IsTableStateReady() then
        if DEBUG_MODE then
