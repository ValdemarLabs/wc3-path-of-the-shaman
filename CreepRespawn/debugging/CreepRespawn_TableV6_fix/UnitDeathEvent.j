...
globals
    ...
    private boolean array UnitDeathEvent_CallbackHasConditions
...
private function UnitDeathEvent_RunCallback takes integer callbackIndex returns nothing
    local trigger callbackTrigger = UnitDeathEvent_Callbacks[callbackIndex]
    ...
    // Code callbacks created by UnitDeathEvent_Register contain actions only
    // and are executed directly. Externally supplied triggers retain their
    // conditions and are evaluated before execution.
    if UnitDeathEvent_CallbackHasConditions[callbackIndex] then
...
private function UnitDeathEvent_DispatchUnits takes unit dyingUnit, unit killingUnit returns nothing
    ...
    loop
        ...
        call UnitDeathEvent_RunCallback(callbackIndex)
...
private function UnitDeathEvent_AddCallbackTrigger takes trigger callbackTrigger, boolean hasConditions returns nothing
    if callbackTrigger == null then
    ...
    set UnitDeathEvent_CallbackHasConditions[UnitDeathEvent_CallbackCount] = hasConditions
...
function UnitDeathEvent_RegisterTrigger takes trigger callbackTrigger returns nothing
    call UnitDeathEvent_AddCallbackTrigger(callbackTrigger, true)
endfunction
...
function UnitDeathEvent_Register takes code callback returns nothing
    ...
    call UnitDeathEvent_AddCallbackTrigger(callbackTrigger, false)
    set callbackTrigger = null
endfunction
