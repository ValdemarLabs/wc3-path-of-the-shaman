...
- Updated `_developer/Design Plans/Warcraft III 3.0 Systems Upgrade Plan.md` to replace separate feature test maps with a disabled, `/debug`-driven validation harness inside a disposable copy of the complete PotS map, preserving real rect, Object Editor, initialization, UI, import, and cross-system dependencies. Established the `DEquipment` aggregate bonus layer as the first runtime upgrade, ahead of the higher-risk native inventory bridge.
