...
- Updated the Warcraft III 3.0 systems plan, documentation index, P0/P1 import instructions, and new P2 validation log to start P2 with the independently reversible SpeciFX slice. Dynamic minimap generation and doodad colors remain manual World Editor work, while cooldown migration remains behind its combat-semantics gate.
- Verified the new P2 effect API slice and harness with focused transformed-source `pjass` checks against the active build-24268 Blizzard scripts; full JassHelper import, model behavior, cleanup, and two-client validation remain manual.
...
- Updated `_developer/Design Plans/Warcraft III 3.0 Upgrade/Warcraft III 3.0 Systems Upgrade Plan.md` and its P0/P1 validation log to mark repository-side Foundation/P0/P1 implementation complete, distinguish remaining manual validation from conditional or optional work, and add exact camera-ownership test and rollback steps. No P2 implementation has started.
