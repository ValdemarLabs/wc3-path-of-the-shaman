...
- Reframed `_developer/Design Plans/Story and Quest Design.md` as the narrative bible with a table of contents, five-act story treatment, character arcs, upper-level dialogue flows, story-status markings, consequence map, and open canon decisions. Moved the prior technical inventory into `_developer/Design Plans/Story and Quest Implementation Ledger.md`, added navigation between both files, and marked `_developer/Design Plans/PotS_Story_Design_Guide.md` as incorporated design provenance so the three documents no longer compete as sources of current truth.
