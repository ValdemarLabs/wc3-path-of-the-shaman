...
- Updated `Debug/Warcraft300P2TestHarness.j`, `Debug/Warcraft300TestHarness.j`, and `Debug/DebugCommands.j` with local minimap source/view/refresh diagnostics and a guarded single-doodad color probe that requires an explicit original color ID for reset.
- Updated the Warcraft III 3.0 systems plan and both validation logs with the native/imported minimap matrix, the runtime-switching uncertainty after `BlzChangeMinimapTerrainTex`, and the remaining P1 doodad authoring, persistence, and pathing gates. `W3-HL-DOODADS` remains in progress rather than complete.
...
- Updated the Warcraft III 3.0 systems plan, documentation index, P0/P1 import instructions, and P2 validation log to start P2 with independently reversible SpeciFX and DynamicMinimap slices. Minimap behavior and doodad colors still require manual World Editor/runtime evidence, while cooldown migration remains behind its combat-semantics gate.
- Verified the new P2 effect APIs, DynamicMinimap source-selection path, P2 harness, and doodad color probe with focused transformed-source `pjass` checks against the active build-24268 Blizzard scripts; full JassHelper import and runtime/two-client behavior remain manual.
...
- Run the resettable doodad color probe only on a reviewed disposable placement with its exact original World Editor color ID, then verify single-instance scope, reset, pathing, save/reopen, and recreation behavior.
