...
globals
    ...
    private constant real UPDATE_INTERVAL = 0.10  // How often to check for updates (default; 0.1)
    ...
    private constant real TILE_SIZE = 128.0  // Size of one tile in world units (128.0)
    ...
    private integer lastChunkX = -1
    private integer lastChunkY = -1
    private rect currentBoundsRect = null
    private rect originalCameraBounds = null

    // Mapping Y drift correction
    private real MAP_WIDTH  = MAP_WORLD_MAX_X - MAP_WORLD_MIN_X
    private real MAP_HEIGHT = MAP_WORLD_MAX_Y - MAP_WORLD_MIN_Y
...
private function UpdateMinimapAndBounds takes integer chunkX, integer chunkY, integer mapTileX, integer mapTileY returns nothing
    local string texturePath
    local real chunkSizeMap
    local real halfWorld
    local real centerX
    local real centerY
    local real minX
    local real minY
    ...
    if chunkX == lastChunkX and chunkY == lastChunkY then
        return
    endif
    
    set lastChunkX = chunkX
    set lastChunkY = chunkY
    
    // Change minimap texture using chunk coordinates (256-tile system)
    set texturePath = GetMinimapTexturePath(chunkX, chunkY, currentChunkSize)
    call BlzChangeMinimapTerrainTex(texturePath)
    
    set chunkSizeMap = I2R(currentChunkSize) * TILE_SIZE
    set halfWorld = chunkSizeMap * 0.5

    set centerX = MAP_WORLD_MIN_X + I2R(mapTileX) * TILE_SIZE + halfWorld
    set centerY = MAP_WORLD_MIN_Y + I2R(mapTileY) * TILE_SIZE + halfWorld

    set minX = centerX - halfWorld
    set minY = centerY - halfWorld
    set maxX = centerX + halfWorld
    set maxY = centerY + halfWorld
...
private function PeriodicUpdate takes nothing returns nothing
    local real camX
    local real camY
    local integer mapTileX
    local integer mapTileY
    local integer chunkX
    local integer chunkY
    local real mapToChunk = I2R(CHUNK_COORDINATE_SYSTEM) / I2R(MAP_SIZE_TILES)
    local real chunkToMap = I2R(MAP_SIZE_TILES) / I2R(CHUNK_COORDINATE_SYSTEM)
    local integer chunkSize256 = R2I(I2R(currentChunkSize) * mapToChunk)
    ...
    // World → map tile
    set mapTileX = R2I((camX - MAP_WORLD_MIN_X) / TILE_SIZE)
    set mapTileY = R2I((camY - MAP_WORLD_MIN_Y) / TILE_SIZE)

    // Clamp map tiles
    if mapTileX < 0 then
        set mapTileX = 0
    elseif mapTileX > MAP_SIZE_TILES - 1 then
        set mapTileX = MAP_SIZE_TILES - 1
    endif

    if mapTileY < 0 then
        set mapTileY = 0
    elseif mapTileY > MAP_SIZE_TILES - 1 then
        set mapTileY = MAP_SIZE_TILES - 1
    endif

    // Map → chunk space
    set chunkX = R2I(I2R(mapTileX) * mapToChunk)
    set chunkY = R2I(I2R(mapTileY) * mapToChunk)

    // Center in chunk space
    set chunkX = chunkX - (chunkSize256 / 2)
    set chunkY = chunkY - (chunkSize256 / 2)

    // Snap to grid
    set chunkX = (chunkX / currentGridStep) * currentGridStep
    set chunkY = (chunkY / currentGridStep) * currentGridStep

    // Clamp chunk coordinates
    if chunkX < 0 then
        set chunkX = 0
    elseif chunkX > CHUNK_COORDINATE_SYSTEM - chunkSize256  then
        set chunkX = CHUNK_COORDINATE_SYSTEM - chunkSize256 
    endif

    if chunkY < 0 then
        set chunkY = 0
    elseif chunkY > CHUNK_COORDINATE_SYSTEM - chunkSize256  then
        set chunkY = CHUNK_COORDINATE_SYSTEM - chunkSize256 
    endif

    // Chunk → map tile (ONCE)
    set mapTileX = R2I(I2R(chunkX) * chunkToMap)
    set mapTileY = R2I(I2R(chunkY) * chunkToMap)

    // Update
    call UpdateMinimapAndBounds(chunkX, chunkY, mapTileX, mapTileY)
endfunction
...
private function Init takes nothing returns nothing
    ...
    // Initial update uses SAME pipeline
    call DynamicMinimap_ForceUpdate()
