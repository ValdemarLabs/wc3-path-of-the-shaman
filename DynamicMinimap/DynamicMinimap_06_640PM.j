...
private function InitFrames takes nothing returns nothing
    ...
    // Create border backdrop frame for enlarged minimap (Blizzard-style)
    // Border is sibling of minimap, not child - children are always above parent
    // Note: Minimap frame is part of GAME UI, so we cant have minimap border as child of minimap
    if ENABLE_BORDER_FRAME then
        // TEST from Refored UI Designer
        set minimapBorderFrame = BlzCreateFrameByType("BACKDROP", "MinimapBorderBackdrop", BlzGetFrameByName("ConsoleUIBackdrop", 0), "", 1)
...
private function Init takes nothing returns nothing
    ...
    // Store original camera bounds for full map mode
    ...
    // Delay frame initialization until after map load
