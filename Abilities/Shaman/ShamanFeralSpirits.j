/**
    ...
    Description:
    Registers Feral Spirits summons as controlled companion units with
    Strength/Agility scaling, without consuming the normal companion party
    limit.
...
private function ApplyFeralBond takes unit summoner, unit wolf returns nothing
    ...
    local real strength = ShamanCommon_GetStat(summoner, ShamanCommon_STAT_STRENGTH)
    local real agility = ShamanCommon_GetStat(summoner, ShamanCommon_STAT_AGILITY)
    ...
    call BlzSetUnitMaxHP(wolf, R2I(I2R(maxLife) * talentMultiplier + strength * 5.00 + agility * 2.00))
    call SetUnitState(wolf, UNIT_STATE_LIFE, GetUnitState(wolf, UNIT_STATE_MAX_LIFE))
    call BlzSetUnitBaseDamage(wolf, R2I(I2R(damage) * talentMultiplier + strength * 0.25 + agility * 0.45), 0)
endfunction
